...
Repeat
	...
	If MenuOpen Or InvOpen Or OtherOpen <> Null Or ConsoleOpen Or SelectedDoor <> Null Or SelectedScreen <> Null Or Using294 Then
		FPSfactor = 0
	EndIf
	...
	;===========================
	;== Main Menu Logic Block ==
	;===========================
	If MainMenuOpen Then
	
		;== Start background video if not active ==
		If Not MenuBGActive Then
			MenuVideoIndex = 0
			NextMenuBGVideo()
		EndIf
	
		;== Play breathing audio once ==
	...
	Else
		;== Clean up background video when leaving menu ==
		If MenuBGActive Then StopMenuBGVideo()
...
Function PlayStartupVideos()
	...
	; Screen Ratio
	Local screenRatio# = Float(RealGraphicWidth) / Float(RealGraphicHeight)
	...
	Repeat
		Local baseFile$ = "GFX\menu\loadingvideos\video" + index
		Local videoExt$ = ""

		If FileType(baseFile + ".avi") = 1 Then
			videoExt = ".avi"
		Else
			Exit ; No more videos
		EndIf

		Local videoPath$ = baseFile + videoExt
		DebugLog "Playing video: " + videoPath

		; Get video dimensions
		BlitzMovie_Open(videoPath)
		Local moview% = BlitzMovie_GetWidth() 
		Local movieh% = BlitzMovie_GetHeight()
		BlitzMovie_Close()

		; Load video into image buffer
		Local image = CreateImage(moview, movieh)
		BlitzMovie_OpenDecodeToImage(videoPath, image, False)
		BlitzMovie_Play()

		; External audio support
		Local SplashScreenAudio% = 0
		If FileType(baseFile + "sound.ogg") = 1 Then
			SplashScreenAudio = StreamSound_Strict(baseFile + "sound.ogg", SFXVolume, 0)
		ElseIf FileType(baseFile + "sound.mp3") = 1 Then
			SplashScreenAudio = StreamSound_Strict(baseFile + "sound.mp3", SFXVolume, 0)
		EndIf

		; Aspect ratio correction for Quad
		Local videoRatio# = Float(moview) / Float(movieh)
		Local scaleX# = 1.0
		Local scaleY# = 1.0
		...
		ScaleEntity Quad, scaleX, scaleY, 1.0

		; Playback loop (Blitz3D-safe NOT workaround)
		Repeat
			Cls
			ProjectImage(image, RealGraphicWidth, RealGraphicHeight, Quad, Texture)
			Flip
			Delay(33)
		Until GetKey() Or (SplashScreenAudio <> 0 And IsStreamPlaying_Strict(SplashScreenAudio) = False)

		If SplashScreenAudio <> 0 Then StopStream_Strict(SplashScreenAudio)
		BlitzMovie_Stop()
		BlitzMovie_Close()
		FreeImage image
		Cls
