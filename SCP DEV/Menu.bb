...
Function UpdateMainMenu()
	...
	;== OPTIONAL: draw menu background image (disabled) ==
	;DrawImage(MenuBack, 0, 0)

	;== SCP-173 blinking Easter egg (disabled) ==
	...
	Else
		...
		Select MainMenuTab
			...
			Case 2 ;load game
				...
				If MenuVideoInitialized
					FreeImage MenuVideoImg
					FreeTexture MenuVideoTex
					FreeEntity MenuVideoQuad
					FreeEntity MenuVideoCam
					MenuVideoInitialized = False
				EndIf
...
Function DrawOptionsTooltip(x%,y%,width%,height%,option$,value#=0,ingame%=False)
	...
	Select Lower(option$)
		...
		Case "fov"
			...
			txt = txt + "(FOV Addon v2.2 - Made by TheHappyJammer)"
			R = 255
			G = 255
			B = 255
